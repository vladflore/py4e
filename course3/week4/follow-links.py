...
# http://py4e-data.dr-chuck.net/known_by_Kisha.html Find the link at position 18 (the first name is 1). Follow that
# link. Repeat this process 7 times. The answer is the last name that you retrieve. Hint: The first character of the
# name of the last page that you will load is: T Strategy The web pages tweak the height between the links and hide
...
if len(url) < 1:
    ...
    url = 'http://py4e-data.dr-chuck.net/known_by_Kisha.html'
...
while it <= icount:
    ...
    html = urllib.request.urlopen(link_to_follow, context=ctx).read()
    soup = BeautifulSoup(html, 'html.parser')
    target_tag = soup('a')[iposition - 1]
    if it == icount:
        print(target_tag.get('href', None))
        print(target_tag.contents[0])
    link_to_follow = target_tag.get('href', None)
    it = it + 1
